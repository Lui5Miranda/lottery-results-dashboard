import { InfosLoto } from "../InfosLoto/InfosLoto";
...
export function Sidebar({ handleLoteria, loteriaData }: SidebarProps) {
  const formatarTipoJogo = (tipo: string) => `--${tipo.replace(/_/g, "-")}`;

  const formatarNomeJogo = (tipo: string) => `${tipo.replace(/_/g, " ")}`;

  return (
    <section
      className={`text-white relative overflow-hidden flex-shrink-0 
        w-full h-[50vh] md:w-[50vw] md:h-screen
        bg-[var(${formatarTipoJogo(loteriaData.tipoJogo)})]
      `}
      style={{
        backgroundColor: `var(${formatarTipoJogo(loteriaData.tipoJogo)})`,
      }}
    >
      <div
        className="flex flex-col justify-between h-full w-full p-6 items-center 
        md:ml-[10%] md:items-start"
      >
        <Select handleLoteria={handleLoteria} />
